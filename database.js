...
export async function initializeDatabase() {
  ...
  // --- FORCE MIGRATION: Drop users table if it has old CHECK constraints ---
  try {
    // Try to insert a status that would fail the old CHECK constraint
    // If it fails, we know we need to drop and recreate.
    db.run("INSERT INTO users (id, name, email, role, status) VALUES ('temp', 'temp', 'temp@temp.com', 'USER', 'Enabled')");
    db.run("DELETE FROM users WHERE id = 'temp'");
  } catch (e) {
    if (e.message.includes('CHECK constraint failed')) {
      console.log('🔄 Old users table detected with restrictive constraints. Migrating...');
  ...
  // --- 2026-GRADE AUTHORITATIVE PIPELINE TABLES ---
  db.run(`
  ...
  db.run(`
    CREATE TABLE IF NOT EXISTS activities (
      ...
      contact_id TEXT,
      tenant_id TEXT DEFAULT 'shimchatalent',
      type TEXT NOT NULL, -- CREATED, CALL, SMS, NOTE, UPDATED
      actor_id TEXT NOT NULL, -- email or SYSTEM
      content TEXT,
      metadata TEXT, -- JSON
      created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  ...
  const tables = ['users', 'contacts', 'reports', 'automations', 'messages', 'call_logs', 'intake_log', 'activities', 'policies'];
  ...
  db.run(`
    INSERT OR IGNORE INTO policies (id, tenant_id, key, value)
    VALUES ('default_dedup', 'shimchatalent', 'DEDUP_STRATEGY', '{"priority": ["EMAIL", "PHONE"], "mode": "REJECT"}')
  `);
  db.run(`
    INSERT OR IGNORE INTO policies (id, tenant_id, key, value)
    VALUES ('default_validation', 'shimchatalent', 'VALIDATION_RULES', '{"require_identity": true, "mandatory_fields": ["name"]}')
  `);

  // Ensure default tenant exists
  db.run(`
    INSERT OR IGNORE INTO tenants (id, name, slug, status) 
    VALUES ('shimchatalent', 'Tropos Main', 'shimchatalent', 'ACTIVE')
  `);
