...
const getTenantSlug = () => {
    if (typeof window === 'undefined') return 'shimchatalent';
    const path = window.location.pathname; // e.g. /shimchatalent/dashboard
...
export async function createContact(contact: any, source: string = 'UI') {
...
export async function fetchActivities(contactId: string) {
    return apiCall(`/activities/${contactId}`);
}
...
export async function simulateCall(phoneNumber: string, status: string, name: string = 'Simulator', sessionId?: string) {
    return apiCall('/debug/simulate-call', {
        method: 'POST',
        body: JSON.stringify({ phoneNumber, status, name, sessionId }),
    });
}
