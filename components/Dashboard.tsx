...
import { ClientData, AppUser } from '../types';
...
const Dashboard: React.FC<DashboardProps> = ({ currentUser }) => {
  const { snapshot } = useGridEngine();

  const data = React.useMemo(() => {
    if (currentUser.role === 'ADMIN') return snapshot;
    return snapshot.filter(client => client.assignedTo === currentUser.email);
