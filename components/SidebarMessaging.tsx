...
const SidebarMessaging: React.FC<SidebarMessagingProps> = ({ currentUser, allUsers, onClose, activeRowId }) => {
  const { getRowById } = useGridEngine();
  const activeRow = activeRowId ? getRowById(activeRowId) : null;
  const activeClientName = activeRow?.name;
