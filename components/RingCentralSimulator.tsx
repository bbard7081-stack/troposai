...
const RingCentralSimulator: React.FC = () => {
    ...
    const handleSimulateCall = async (status: string) => {
        ...
        try {
            ...
            const result = await api.simulateCall(phone, status, name, sid || undefined);
            const actualSid = result.sessionId;
            if (status === 'Ringing') setSessionId(actualSid);

            setLastAction(`Simulated ${status} for ${phone} (SID: ${actualSid})`);
