...
interface RingCentralModuleProps {
  ...
  onHangUp: (reason?: 'declined' | 'missed' | 'disconnected') => void;
  onRefreshData?: () => void;
}
...
const RingCentralModule: React.FC<RingCentralModuleProps> = ({
  ...
  const { snapshot, engine } = useGridEngine();
              ...
              <div className="flex flex-col items-center text-center space-y-6">
                ...
                <div className="space-y-1">
                  <p className="text-2xl font-black text-white tracking-tight">
                    {(() => {
                      const contact = snapshot.find(c => c.phone === activeCall.phoneNumber);
                      return contact ? contact.name : activeCall.phoneNumber;
                    })()}
                  </p>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <button
                        onClick={() => {
                          const contact = snapshot.find(c => c.phone === activeCall.phoneNumber);
                          if (contact) {
                            engine?.updateContact(contact.id, {
                              missedCall: true,
                              declinedCall: true,
                              notes: (contact.notes || '') + `\n[${new Date().toLocaleString()}] Call declined`
                            }, 'call');
                          }
                          onHangUp('declined');
                        }}
                        ...
                        onClick={() => {
                          ...
                          if (contact) {
                            engine?.updateContact(contact.id, {
                              ...
                              notes: (contact.notes || '') + `\n[${new Date().toLocaleString()}] Call accepted by ${currentUser.name}`
                            }, 'call');
                          }
                          onTakeCall(activeCall.phoneNumber);
                        }}
