...
type AdminTab = 'overview' | 'users' | 'backups' | 'settings' | 'simulator';
...
const AdminPanel: React.FC<AdminPanelProps> = ({ users, data = [], onUpdateUser, onAddUser, onDeleteUser, onImpersonate, currentUser }) => {
  ...
  const openAddModal = () => {
    ...
    setFormData({ name: '', email: '', ringCentralEmail: '', role: 'USER', team: '' });
    setIsModalOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    if (selectedUser) {
      ...
      try {
        ...
        const inviteLink = `${window.location.origin}/shimchatalent`;
        ...
        const emailHtml = `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                ...
                <a href="${inviteLink}" style="display: inline-block; background-color: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">Launch Workspace</a>
                <p style="color: #64748b; font-size: 12px; margin-top: 24px;">If the button doesn't work, copy this link: ${inviteLink}</p>
            </div>
          `;

        await api.sendEmail(formData.email, emailSubject, emailHtml);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
                    ...
                    <tr key={user.id} className="hover:bg-slate-50/50 transition-colors group">
                      ...
                      <td className="px-8 py-5">
                        <div className="flex flex-col items-start space-y-1">
                          <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${user.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700 animate-pulse'}`}>
                            {user.status}
                          </span>
                      ...
                      <td className="px-8 py-5">
                        <select
                          ...
                          className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider outline-none cursor-pointer border-transparent hover:border-slate-300 border transition-all ${user.role === 'ADMIN' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'}`}
                        >
              ...
              <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl p-6 text-white">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-mono">/shimchatalent</span>
                  </div>
                  <div className="flex justify-between">
      ...
      {/* Simulator Tab */}
      {activeTab === 'simulator' && <RingCentralSimulator />}
            ...
            <div className="flex justify-between items-center mt-8">
              ...
              <div className="flex space-x-3">
                <button
                  ...
                  className="flex-1 px-4 py-3 bg-slate-100 text-slate-600 rounded-2xl hover:bg-slate-200 font-bold transition-all text-sm"
                  disabled={isSending}
                >
                  Cancel
                </button>
